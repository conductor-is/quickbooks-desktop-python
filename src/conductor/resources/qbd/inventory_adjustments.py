        ...
        *,
        ...
        Args:
          ...
          transaction_date_from: Filter for inventory adjustments whose `date` field is on or after this date, in
              ISO 8601 format (YYYY-MM-DD). QuickBooks Desktop interprets this date-only value
              in the host machine’s local timezone; i.e., midnight in the timezone of the
          ...
          transaction_date_to: Filter for inventory adjustments whose `date` field is on or before this date,
              in ISO 8601 format (YYYY-MM-DD). QuickBooks Desktop interprets this date-only
              value in the host machine’s local timezone; i.e., midnight in the timezone of
              the end-user's computer running QuickBooks Desktop.

          updated_after:
              Filter for inventory adjustments updated on or after this date/time. Format:
          ...
          updated_before:
              Filter for inventory adjustments updated on or before this date/time. Format:
              ISO 8601. Accepts date-only (YYYY-MM-DD), datetime without timezone
              (YYYY-MM-DDTHH:mm:ss), or datetime with timezone (YYYY-MM-DDTHH:mm:ss±HH:mm).
              Date-only and timezone-less datetimes are passed through for QuickBooks Desktop
              to interpret in the host machine’s local timezone. If the datetime includes a
