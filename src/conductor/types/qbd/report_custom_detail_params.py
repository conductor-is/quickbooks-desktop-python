...
__all__ = ["ReportCustomDetailParams"]


class ReportCustomDetailParams(TypedDict, total=False):
    include_columns: Required[
        Annotated[
            List[
                Literal[
                    "account",
    ...
    **IMPORTANT**: When this parameter is present, QuickBooks Desktop omits its
    ...
    report_type: Annotated[Literal["custom_transaction_detail"], PropertyInfo(alias="reportType")]
    """The custom detail report type to retrieve.

    This endpoint supports only `custom_transaction_detail`, so this parameter is
    optional and defaults to `custom_transaction_detail`.
    """
